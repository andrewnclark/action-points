defmodule ActionPointsWeb.UserAuth do
  use ActionPointsWeb, :verified_routes
  ...
  @remember_me_cookie "_action_points_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ActionPointsWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_scope_for_user(conn, _opts) do
    with {token, conn} <- ensure_user_token(conn),
      ...
      conn
      |> assign(:current_scope, Billing.with_balance(Scope.for_user(user)))
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      ActionPointsWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule ActionPointsWeb.PageLive do
        use ActionPointsWeb, :live_view

        on_mount {ActionPointsWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ActionPointsWeb.UserAuth, :require_authenticated}] do
  ...
  defp mount_current_scope(socket, session) do
    Phoenix.Component.assign_new(socket, :current_scope, fn ->
      ...
      Billing.with_balance(Scope.for_user(user))
    end)
  end
