...
- The union landed whole, but the rules arrive ticket by ticket: `weekday`, `relative_day`, complete `absolute` dates and `span_end` resolve so far; `duration` and partial dates resolve to nothing until their tickets land.
